...
version, before committing. Do not re-run it on an already-quantised file, that compounds the loss for almost no further saving.
...
- The alternative is WebP, which would roughly third the total. `cwebp` is not
  ...
  option today. **If the page ever needs to get lighter, that is the lever**,   `brew install webp`, convert the whole `images/` folder, and update the
...
**`frontyard.jpeg` carried GPS.** It is the first photo in this batch that did: the plant photos had been through the garden app's upload path, which strips it,
...
26 pieces, all transparent PNG. **Do not run these through `tools/prep-photo.py`**, it writes JPEG and would flatten the alpha to a white box. The pipeline for
